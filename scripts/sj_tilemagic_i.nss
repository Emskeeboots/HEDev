...
const string AA_VAR_TILEMAGIC_PLACE     = "aa_tilemagic_place";
const string AA_VAR_TILEMAGIC_TYPE      = "aa_tilemagic_type";
const string AA_VAR_TILEMAGIC_MIST      = "aa_tilemagic_mist";
...
//  - nVFX:        SJ_TILEMAGIC_* / SJ_TILEMAGIC_FEATURE_* constant
//  - fZ:           height of the tile
//  - bNormalise:   TRUE (to compensate for tile's internal offset) or FALSE
//  - nRotation:    SJ_ROTATION_* constant
void SJ_TileMagic_AddStyleToPalette(object oPalette, string sStyle, int nVFX, float fZ=0.0, int bNormalise=TRUE, int nRotation=0);

// Removes all TileMagic tiles in oArea.
//  - oArea:        area containing TileMagic tiles
//  - sType:        remove a specific type or if sType="", any type
void SJ_TileMagic_ClearArea(object oArea, string sType="");

// Removes any TileMagic tiles in oArea that fall within the specified border.
//  - oArea:        area containing TileMagic tiles
//  - nWidth:       width of border to be cleared
//  - sType:        remove a specific type or if sType="", any type
void SJ_TileMagic_ClearBorder(object oArea, int nWidth=1, string sType="");
...
//  - sType:        remove a specific type or if sType="", any type
void SJ_TileMagic_ClearRange(object oArea, int nColumn=0, int nRow=0, int nColumns=1, int nRows=1, string sType="", int bMist=FALSE);
...
//  - sType:        remove a specific type or if sType="", any type
void SJ_TileMagic_ClearTile(object oArea, int nColumn = 0, int nRow = 0, string sType="", int bMist=FALSE);
...
//  - bDestroy      TRUE - destroy object that is converted to tile.
void SJ_TileMagic_ConvertObjectToTile(object oObject = OBJECT_SELF, int bDestroy=TRUE);

// Covers oArea with TileMagic tiles. The tiles have a vertical offset of fZ
// modified by nVFX's internal offset unless using bNormalise.
//  - oArea:        area to be tiled
//  - nVFX:         SJ_TILEMAGIC_ORDINARY_* / SJ_TILEMAGIC_FEATURE_* constant
//  - fZ:           height of the tile
//  - bNormalise:   TRUE (to compensate for tile's internal offset) or FALSE
//  - nRotation:    SJ_ROTATION_* constant
//  - sRef:         resref of placeable
//  - sType:        string identifier used to avoid collisions and stack multiple affects on one tile
void SJ_TileMagic_CoverArea(object oArea, int nVFX, float fZ=0.0, int bNormalise=TRUE, int nRotation=0, string sRef=SJ_RES_INVISIBLE_OBJECT, string sType="", int bMist=FALSE);
...
// of fZ modified by nVFX's internal offset unless using bNormalise.
//  - oArea:        area to be tiled
//  - nVFX:         SJ_TILEMAGIC_ORDINARY_* / SJ_TILEMAGIC_FEATURE_* constant
...
//  - sRef:         resref of placeable
//  - sType:        string identifier used to avoid collisions and stack multiple affects on one tile
void SJ_TileMagic_CoverBorder(object oArea, int nVFX, int nWidth=1, float fZ=0.0, int bNormalise=TRUE, int nRotation=0, string sRef=SJ_RES_INVISIBLE_OBJECT, string sType="", int bMist=FALSE);

// Covers oArea with TileMagic tiles in quarters to avoid TMI errors. The tiles
// have a vertical offset of fZ modified by nVFX's internal offset unless using
// bNormalise.
...
//  - bNormalise:   TRUE (to compensate for tile's internal offset) or FALSE
//  - sType:        string identifier used to avoid collisions and stack multiple affects on one tile
void SJ_TileMagic_CoverLargeArea(object oArea, int nVFX, float fZ = 0.0, int bNormalise = TRUE, int nRotation=0, string sRef=SJ_RES_INVISIBLE_OBJECT, string sType="", int bMist=FALSE);
...
// fZ modified by nVFX's internal offset unless using bNormalise.
//  - oArea:        area to be tiled
//  - nVFX:        SJ_TILEMAGIC_ORDINARY_* / SJ_TILEMAGIC_FEATURE_* constant
//  - nColumn:      column index of bottom left corner of range
//  - nRow:         row index of bottom left corner of range
...
//  - nRotation:    SJ_ROTATION_* constant
//  - sType:        string identifier used to avoid collisions and stack multiple affects on one tile
void SJ_TileMagic_CoverRange(object oArea, int nVFX, int nColumn=0, int nRow=0, int nColumns=1, int nRows=1, float fZ=0.0, int bNormalise=TRUE, int nRotation=0, string sRef=SJ_RES_INVISIBLE_OBJECT, string sType="", int bMist=FALSE);

// Covers the equivalent of a single tile (10m x 10m) centred at the specified
// spot (x, y). The tile has a vertical offset of fZ modified by nVFX's
...
//  - sType:        string identifier used to avoid collisions and stack multiple affects on one tile
void SJ_TileMagic_CoverSpot(object oArea, int nVFX, float fX, float fY, float fZ=0.0, int bNormalise=TRUE, float Rotation=0.0, string sRef=SJ_RES_INVISIBLE_OBJECT, string sType="", int bMist=FALSE);

// Covers a single tile in oArea with a TileMagic tile. The tile has a vertical
// offset of fZ modified by nVFX's internal offset unless using bNormalise.
//  - oArea:        area containing tile
//  - nVFX:         the VFX to apply to the placeable, SJ_TILEMAGIC_ORDINARY_* / SJ_TILEMAGIC_FEATURE_* constant
//  - nColumn:      column index tile
//  - nRow:         row index tile
//  - fZ:           height of the tile
//  - bNormalise:   TRUE (to compensate for tile's internal offset) or FALSE
//  - nRotation:    SJ_ROTATION_* constant
//  - sRef:         string resref of tilemagic placeable
//  - sType:        string identifier used to avoid collisions and stack multiple affects on one tile
void SJ_TileMagic_CoverTile(object oArea, int nVFX, int nColumn, int nRow, float fZ=0.0, int bNormalise=TRUE, int nRotation=0, string sRef=SJ_RES_INVISIBLE_OBJECT, string sType="", int bMist=FALSE);
...
object SJ_TileMagic_CreateTile(int nVFX, location lTile, string sTag, string sRef=SJ_RES_INVISIBLE_OBJECT, string sType="", int bMist=FALSE);
...
// visualeffects.2da. Returns nVFX unmodified in all other circumstances.
int SJ_TileMagic_GetCorrectedConstant(int nVFX);
...
float SJ_TileMagic_GetNormalisedHeight(int nVFX, float fZ);
...
string SJ_TileMagic_TileToString(object oArea, string sTag, int nVFX=0);
...
void SJ_TileMagic_AddStyleToPalette(object oPalette, string sStyle, int nVFX, float fZ=0.0, int bNormalise=TRUE, int nRotation=0)
{
    ...
    if(sDebug == "")
    {
        ...
        SetLocalInt(oPalette, sStyle + SJ_VAR_TILEMAGIC_TILE, nVFX);
...
{
    // get area dimensions
    int nColumns = GetAreaSize(AREA_WIDTH, oArea);
    int nRows = GetAreaSize(AREA_HEIGHT, oArea);
    ...
    for(nCurColumn = 0; nCurColumn < nColumns; nCurColumn++)
    {
        ...
        {
            SJ_TileMagic_ClearTile(oArea, nCurColumn, nCurRow, sType);
        }
    }
    // Magus - when the area is cleared, enable Area's Enter Event to resestablish tilemagic
    DeleteLocalInt(oArea, "sj_tilemagic_done");
}
...
void SJ_TileMagic_CoverLargeArea(object oArea, int nVFX, float fZ = 0.0, int bNormalise = TRUE, int nRotation=0, string sRef=SJ_RES_INVISIBLE_OBJECT, string sType="", int bMist=FALSE)
{
    ...
    int nCols = GetAreaSize(AREA_WIDTH, oArea);
    int nRows = GetAreaSize(AREA_HEIGHT, oArea);
    ...
    AssignCommand(OBJECT_SELF, SJ_TileMagic_CoverRange(oArea, nVFX, 0, 0, nCol, nRow, fZ, bNormalise, nRotation, sRef, sType, bMist));
    AssignCommand(OBJECT_SELF, SJ_TileMagic_CoverRange(oArea, nVFX, nCol, 0, (nCols - nCol), nRow, fZ, bNormalise, nRotation, sRef, sType, bMist));
    AssignCommand(OBJECT_SELF, SJ_TileMagic_CoverRange(oArea, nVFX, 0, nRow, nCol, (nRows - nRow), fZ, bNormalise, nRotation, sRef, sType, bMist));
    AssignCommand(OBJECT_SELF, SJ_TileMagic_CoverRange(oArea, nVFX, nCol, nRow, (nCols - nCol), (nRows - nRow), fZ, bNormalise, nRotation, sRef, sType, bMist));
}
...
void SJ_TileMagic_ClearRange(object oArea, int nColumn = 0, int nRow = 0, int nColumns = 1, int nRows = 1, string sType="", int bMist=FALSE)
{
    ...
    for(nCurColumn = nColumn; nCurColumn < nColumn + nColumns; nCurColumn++)
    {
        ...
        {
            SJ_TileMagic_ClearTile(oArea, nCurColumn, nCurRow, sType, bMist);
        }
    }
}


void SJ_TileMagic_ClearSpot(object oArea, float fX, float fY, string sType="", int bMist=FALSE)
{
    // use unique tag to ID tile node
    string sTag = SJ_TileMagic_GetTagForSpot(oArea, fX, fY);
    object oTile = GetObjectByTag(sTag);

    if(oTile!=OBJECT_INVALID)
    {
        if( sType==""
            ||  sType==GetLocalString(oTile, AA_VAR_TILEMAGIC_TYPE)
          )
        {
            // clear tile
            ...
            if(bMist)
            {
                DestroyObject(GetObjectByTag(sTag+"mist"), SJ_TILEMAGIC_DELAY_CLEAR);
            }
        }
    }
    else
...
void SJ_TileMagic_ConvertObjectToTile(object oObject = OBJECT_SELF, int bDestroy=TRUE)
{
    ...
    int nVFX        = GetLocalInt(oObject, SJ_VAR_TILEMAGIC_TILE);
    string sRef     = GetLocalString(oObject, AA_VAR_TILEMAGIC_PLACE); if(sRef=="")sRef=SJ_RES_INVISIBLE_OBJECT;
    float fZ        = GetLocalFloat(oObject, SJ_VAR_TILEMAGIC_Z);
    string sType    = GetLocalString(oObject, AA_VAR_TILEMAGIC_TYPE);
    int bMist       = GetLocalInt(oObject, AA_VAR_TILEMAGIC_MIST);
    ...
    // adjust height to 1m intervals if flagged
    if(bUseLayer)
    {
        // height increases by 1m per layer
        fZ  += IntToFloat(SJ_TileMagic_GetLayer(oObject));
    }
    // use the height of the waypoint we are converting
    else
    {
    ...
    for(nCurColumn = nColumn; nCurColumn < nColumn + nColumns; nCurColumn++)
    {
        for(nCurRow = nRow; nCurRow < nRow + nRows; nCurRow++)
        {
            SJ_TileMagic_CoverTile(oArea, nVFX, nCurColumn, nCurRow, fZ, bNormalise, nRotation, sRef, sType, bMist);
        }
    }
...
void SJ_TileMagic_CoverArea(object oArea, int nVFX, float fZ=0.0, int bNormalise=TRUE, int nRotation=0, string sRef=SJ_RES_INVISIBLE_OBJECT, string sType="", int bMist=FALSE)
{
    ...
    int nColumns = GetAreaSize(AREA_WIDTH, oArea);
    int nRows = GetAreaSize(AREA_HEIGHT, oArea);
    if(sRef!=SJ_RES_INVISIBLE_OBJECT && !nVFX)
        nVFX=-1;
...
void SJ_TileMagic_CoverBorder(object oArea, int nVFX, int nWidth=1, float fZ=0.0, int bNormalise=TRUE, int nRotation=0, string sRef=SJ_RES_INVISIBLE_OBJECT, string sType="", int bMist=FALSE)
{
    ...
    if(nWidth < 1)
    {
        string sDebug = "SJ_TileMagic_CoverBorder: failed as border width (" + IntToString(nWidth) + ") was invalid.";
        SJ_Debug(SJ_DEBUG_PREFIX_ERROR + sDebug, TRUE);
        return;
    }
    ...
    for(nRow = 0; nRow < nRows; nRow++)
    {
        ...
        for(nColumn = 0; nColumn < nColumns; nColumn++)
        {
            ...
            if(sStyle != "" && sStyle != " " && sStyle != SJ_TILEMAGIC_STYLE_NONE)
            {
                ...
                int nVFX = GetLocalInt(oPalette, sStyle + SJ_VAR_TILEMAGIC_TILE);
                ...
                SJ_TileMagic_CoverTile(oArea, nVFX, nColumn, nRow, fOffset + fZ, bNormalise, nRotation);
            }
        }
    }
}
...
void SJ_TileMagic_CoverSpot(object oArea, int nVFX, float fX, float fY, float fZ=0.0, int bNormalise=TRUE, float fRotation=0.0, string sRef=SJ_RES_INVISIBLE_OBJECT, string sType="", int bMist=FALSE)
{
    // use unique tag to ID tile node
    string sTag = SJ_TileMagic_GetTagForSpot(oArea, fX, fY);
    object oTile = GetObjectByTag(sTag);
    ...
    if(SJ_GetIsSpotValid(oArea, fX, fY))
    {
        ...
        if(     oTile!=OBJECT_INVALID
            &&  GetLocalString(oTile, AA_VAR_TILEMAGIC_TYPE)==sType  // Magus - added "type"
          )
        {
        ...
        if(bNormalise)
        {
            ...
            fZ = SJ_TileMagic_GetNormalisedHeight(nVFX, fZ);
        }
        ...
        // create tile
        oTile = SJ_TileMagic_CreateTile(nVFX, lTile, sTag, sRef, sType, bMist);
    }
    else
    {
        // log an error
        string sDebug =  "SJ_TileMagic_CoverSpot: failed as target (" + SJ_TileMagic_TileToString(oArea, sTag, nVFX) + ") was outwith area.";
        SJ_Debug(SJ_DEBUG_PREFIX_ERROR + sDebug, TRUE);
    }
}
...
void SJ_TileMagic_CoverTile(object oArea, int nVFX, int nColumn, int nRow, float fZ=0.0, int bNormalise=TRUE, int nRotation=0, string sRef=SJ_RES_INVISIBLE_OBJECT, string sType="", int bMist=FALSE)
{
    ...
    if(SJ_GetIsTileValid(oArea, nColumn, nRow))
    {
          ...
          )
        {
            ...
            DestroyObject(oTile, SJ_TILEMAGIC_DELAY_CLEAR);
            // handle mist additions
            oTile = GetObjectByTag(sTag+"mist");
            if(     oTile!=OBJECT_INVALID
                &&  GetLocalString(oTile, AA_VAR_TILEMAGIC_TYPE)==sType  // Magus - added "type"
              )
                DestroyObject(oTile, SJ_TILEMAGIC_DELAY_CLEAR);
    ...
    {
        // log an error
        string sDebug =  "SJ_TileMagic_CoverTile: failed as target (" + SJ_TileMagic_TileToString(oArea, sTag, nVFX) + ") was outwith area.";
        SJ_Debug(SJ_DEBUG_PREFIX_ERROR + sDebug, TRUE);
    }
}
...
object SJ_TileMagic_CreatePalette(string sName, int bUse2DA=FALSE)
{
    ...
    if(bUse2DA)
    {
        ...
        while(sStyle != "")
        {
            ...
            int nVFX = StringToInt(Get2DAString(sName, SJ_TILEMAGIC_2DA_TILE, nIndex));
            ...
            // add data to datapoint
            SJ_TileMagic_AddStyleToPalette(oPalette, sStyle, nVFX, fZ, bNormalise, nRotation);
...
object SJ_TileMagic_CreateTile(int nVFX, location lTile, string sTag, string sRef=SJ_RES_INVISIBLE_OBJECT, string sType="", int bMist=FALSE)
{
    object oTile = CreateObject(OBJECT_TYPE_PLACEABLE, sRef, lTile, FALSE, sTag);

    if(nVFX>-1)
    {
        effect eTile = EffectVisualEffect(SJ_TileMagic_GetCorrectedConstant(nVFX));
        ApplyEffectToObject(DURATION_TYPE_PERMANENT, eTile, oTile);
    }

    // store the VFX so we can subsequently recover all of the tile's properties
    SetLocalInt(oTile, SJ_VAR_TILEMAGIC_TILE, nVFX);
    // Magus
    // store the type so that we can avoid collisions by type (water, fog, vegetation, ground etc...)
    SetLocalString(oTile, AA_VAR_TILEMAGIC_TYPE, sType);

    if(bMist)
    {
        object oMist    = CreateObject(OBJECT_TYPE_PLACEABLE, "x3_plc_mist", lTile, FALSE, sTag+"mist");
        // store the type so that we can avoid collisions by type (water, fog, vegetation, ground etc...)
        SetLocalString(oMist, AA_VAR_TILEMAGIC_TYPE, sType);
    }
...
int SJ_TileMagic_GetCorrectedConstant(int nVFX)
{
    ...
    switch(nVFX)
    {
        ...
        case 348: nVFX = SJ_TILEMAGIC_FEATURE_CITY_MARBLE_ARCH;     break;
        case 430: nVFX = SJ_TILEMAGIC_FEATURE_ITHILLID_TRANSPORTER; break;
        case 431: nVFX = SJ_TILEMAGIC_FEATURE_UNDERDARK_WATERFALL;  break;
        case 441: nVFX = SJ_TILEMAGIC_FEATURE_GRASS_ANT_HILL;       break;
        case 453: nVFX = SJ_TILEMAGIC_FEATURE_CITY_MARBLE_ARCH;     break;
        case 456: nVFX = SJ_TILEMAGIC_FEATURE_CITY_MARKET_1;        break;
    }
...
float SJ_TileMagic_GetNormalisedHeight(int nVFX, float fZ)
{
    ...
    {
        // parse nVFX for special cases
        case 1513:                              fZ -= 7.50; break;  // medrural-grass
        case 1514:                              fZ -= 5.00; break;  // qtno-grass
        case 1497:                                                  // tropical-grass
        case 1498:                                                  // tropical-grass
        case 1511:                                                  // tropical-grass
        case 1512:                                                  // tropical-grass
        case 1600:                              fZ -= 1.00; break;  // ice
        case SJ_TILEMAGIC_ORDINARY_LAVA:                            // 350
        case SJ_TILEMAGIC_ORDINARY_WATER_BLUE_1:                    // 401
        case SJ_TILEMAGIC_ORDINARY_WATER_BLUE_2:                    // 437
        case SJ_TILEMAGIC_FEATURE_LAVA_FOUNTAIN:fZ += 1.00; break;  // 349
        case SJ_TILEMAGIC_ORDINARY_WATER_GREEN: fZ += 2.00; break;  // 461
        case SJ_TILEMAGIC_ORDINARY_PIT:         fZ += 2.40; break;  // 506
    }

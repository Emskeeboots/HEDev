...
//:://////////////////////////////////////////////
//:: Created: Knat (8/2004)
//:: Modified: The Magus (2010 dec 29) moved some constants to aa_inc_constants
//:://////////////////////////////////////////////

// The Magus's PW constants
...
Areas:
    REMOVED
    - the NBDE vault was moved to [OOC] Utility area
  name: _NBDE
...
// HASH RELATED CONSTANTS FOR THE DB AS IMPLEMENTED BY NBDE
/*
   HASH_INDEX
   - Used to safely save to the Bioware DB.
   - everything not in the hash gets considered an illegal character
   - mixed up for additional security
   HASH_PRIME
   - Basis for the hash function
*/
const string HASH_INDEX = "#i!j$k%l{&M/n(o)p=q?r^¤Xs`Tu'v]AwBxCyDzE1F2-G3t;4I}5Y:J6_K7+Z[Lm9N\ l0kOjPhQ,gRfSeHdU8cVbWa.§‡†€Ø";
...
void NBDE_SetCampaignLocation(string sCampaignName, string sVarname, location locLocation, object oPlayer = OBJECT_INVALID);
...
// There is no limit on the length of sVarname (original function is limited to 32 chars)
// sVarname must NOT be unique. you can use the same sVarname with a different data-type
void NBDE_SetCampaignVector(string sCampaignName, string sVarname, vector vVector, object oPlayer = OBJECT_INVALID);
...
// hash related constants originally set in this script moved to _inc_constants
...
string NBDE_GetPlayerKey(object oPC)
{
    ...
    if(sPCID!="")
        return sPCID;
    // Delimiters: * . : _
    // this ID uses Â¤ as a delimeter because it shouldn't be used in tags or resrefs

    // Other characters
    if(!GetIsPC(oPC))
        sPCID   = "NPCÂ¤"+GetTag(oPC)+"Â¤"+GetResRef(oPC)+"Â¤"+GetName(oPC);
    else
    {
        // Setup
        int nPos, nLen;
        sPCID   = GetName(oPC)+"Â¤"+GetPCPlayerName(oPC);
        ...
        while(nPos!=-1)
        {
            sPCID   = GetStringLeft(sPCID,nPos)+"Â§"+GetStringRight(sPCID,nLen-(nPos+1));
        ...
        // Fourth delimiter
        nPos = FindSubString(sPCID, "_");
        while(nPos!=-1)
        {
            sPCID   = GetStringLeft(sPCID,nPos)+"Â¡"+GetStringRight(sPCID,nLen-(nPos+1));
            nPos    = FindSubString(sPCID, "_", nPos+1);
        }
        // Other Characters
        nPos = FindSubString(sPCID, "Ã‹");
        while(nPos!=-1)
        {
            sPCID   = GetStringLeft(sPCID,nPos)+"E"+GetStringRight(sPCID,nLen-(nPos+1));
            nPos    = FindSubString(sPCID, "Ã‹", nPos+1);
        }
        nPos = FindSubString(sPCID, "Ã˜");
        while(nPos!=-1)
        {
            sPCID   = GetStringLeft(sPCID,nPos)+"O"+GetStringRight(sPCID,nLen-(nPos+1));
            nPos    = FindSubString(sPCID, "Ã˜", nPos+1);
        }
    }
...
object NBDE_GetCampaignDatabaseObject(string sCampaignName)
{
  ...
  if(!GetIsObjectValid(oDatabase))
  {
    ...
    {
      WriteTimestampedLogEntry("ERR NBDE> unable to locate '"+NBDE_VAULT_TAG+"' vault container object");
    ...
    if(!GetIsObjectValid(oDatabase))
    {
      WriteTimestampedLogEntry("ERR NBDE> unable to create '"+sCampaignName+"' database object");
      return OBJECT_INVALID;
    }
...
void NBDE_FlushCampaignDatabase(string sCampaignName)
{
  ...
  if(GetIsObjectValid(oVault))
  {
    ...
    if(GetIsObjectValid(oDatabase))
    {
      ...
      // store database
      bSuccess = StoreCampaignObject(sCampaignName, NBDE_DATABASE_ITEM_VARNAME , oDatabase);
      if(!bSuccess)
        WriteTimestampedLogEntry("ERR NBDE> Failed to store: "+sCampaignName);
    }

...
int MaximizeOrEmpower(int nDice, int nNumberOfDice, int nMeta, int nBonus = 0)
{
    ...
    {
       if(!GetLocalInt(GetModule(),"71_SOU_EMPOWER_SPELL_BEHAVIOR"))
       {
...
int ArcaneArcherDamageDoneByBow(int bCrit = FALSE, object oUser = OBJECT_SELF)
{
    ...
    if (GetIsObjectValid(oItem) == TRUE)
    {
        ...
        {
            nDamage = d8(bCrit == TRUE ? 3 : 1);//correct critical hit damage calculation (will enable odd values)
        ...
        {
            nDamage = d6(bCrit == TRUE ? 3 : 1);//correct critical hit damage calculation (will enable odd values)
...
int spellsIsTarget(object oTarget, int nTargetType, object oSource)
{
    ...
    switch (nTargetType)
    {
        ...
        case 2://1.70: added new value now 2 == SPELL_TARGET_SINGLETARGET which is used
        {      //for spells with single target, when these spells are cast at neutral target then they are
            if(!GetIsReactionTypeFriendly(oTarget,oSource))//supposed to hurt target which is not possible
            {                                              //with SPELL_TARGET_STANDARDHOSTILE
                nReturnValue = TRUE;
            }
            else
            {

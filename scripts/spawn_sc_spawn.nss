...
// -------------------------------------------
    ...
    */
    ...
    // Only one of the following scripts will occur for a spawnpoint calling a spawn script
        ...
        else if (nSpawnScript == 11)
        {

            ActionAttack(GetNearestObjectByTag("skelattack"));
            return;
        }
        ...
        else if (nSpawnScript == 12)
        {
            ...
            SetLocalInt(OBJECT_SELF, "PERFORM_NEXT",GetTimeCumulative()+(nDelay*GetLocalInt(GetModule(), "IGMINUTES_PER_RLMINUTE")));
        ...
        {
		ExecuteScript("aid_do_spawn", OBJECT_SELF);
        }
        // Spawn Script 031 ----------------------------------------------------

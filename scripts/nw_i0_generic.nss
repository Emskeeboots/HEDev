...
void AIFlightResponse(object oFrom, int nShout=0, int bClearAllActions=TRUE, float fDistanceFrom=40.0)
{
    ...
    {
        if(CreatureGetIsFlier(OBJECT_SELF,TRUE))
        {

            ApplyEffectToObject(DURATION_TYPE_TEMPORARY,
                                    EffectDisappearAppear(lFleeDest),
                                    OBJECT_SELF,
                                    18.0
                                   );
        }
        else if(GetLocalString(OBJECT_SELF,"X2_SPECIAL_COMBAT_AI_SCRIPT")=="_ai_jump_spec")
        {
            ...
            {
                switch(GetCreatureSize(OBJECT_SELF)) // based on size
                {
            ...
            else if(GetHasFeat(2017))// has spiderclimb?
                fMaxDistance = 320.0; // spiders indoors can jump/climb and reach anywhere

            float xDist   = GetDistanceBetweenLocations(lFleeDest,GetLocation(OBJECT_SELF));
            if(xDist>0.0 && xDist<=fMaxDistance)
            {
                if(GetLocalInt(OBJECT_SELF,"AI_STEALTHY"))
    ...
    if(GetIsObjectValid(oIntruder))
    {
        ...
        if (GetAILevel(OBJECT_SELF) >= AI_LEVEL_HIGH && GetResRef(oIntruder) == "x2_s_bblade")
        {
            ...
            {   //still creature can have a scroll with one of these spell
                ...
                object oNewTarget = ChooseNewTarget(oIntruder);
...
void DetermineSpecialBehavior(object oIntruder = OBJECT_INVALID, int nShout=0)
{
   ...
   //SendMessageToPC(GetFirstPC(), "DetermineSpecialBehavior() Intruder("+IntToString(bIntruder)+") Target("+GetName(oTarget)+")");
      ...
      )
    {
        ...
        else if( DoSkillCheck(oTarget, SKILL_ANIMAL_EMPATHY, nEmpDC+5) )
            nResult = FRIEND;
        // FAMILIAR ----------------------------------------------------
...
void DetermineSpecialHeartbeat()
{
    ...
    else if(fDist<=GetLocalFloat(OBJECT_SELF, "AI_DISTANCE_THREATENED"))
    {
        ...
        else
        {
            ...
            {
                if( DoSkillCheck(oRespond, SKILL_ANIMAL_EMPATHY, GetLocalInt(OBJECT_SELF, "ANIMAL_EMPATHY_DC")) )
                {

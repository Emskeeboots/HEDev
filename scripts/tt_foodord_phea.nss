void main()
{
    int nActive = GetLocalInt (OBJECT_SELF,"order_phea");
...
if (nActive == 0)
{
    ...
    if (GetGold(oPC) >= 500)
       ...
       CreateObject(OBJECT_TYPE_PLACEABLE,sFoodSpawn7,lTarget5,TRUE);
//       CreateObject(OBJECT_TYPE_PLACEABLE,sFoodSpawn2,lTarget4,TRUE);
       ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(iVFX),lTarget5);
//       ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(iVFX),lTarget4);
    ...
    SetLocalInt(OBJECT_SELF,"order_phea", 1);
    DelayCommand(2400.0, SetLocalInt(OBJECT_SELF, "order_phea", 0));

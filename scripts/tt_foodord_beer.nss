void main()
{
    int nActive = GetLocalInt (OBJECT_SELF,"order_beer");
...
if (nActive == 0)
{
    ...
    if (GetGold(oPC) >= 50)
       ...
       CreateObject(OBJECT_TYPE_PLACEABLE,sFoodSpawn5,lTarget9,TRUE);
       CreateObject(OBJECT_TYPE_PLACEABLE,sFoodSpawn5,lTarget10,TRUE);
       ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(iVFX),lTarget9);
       ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(iVFX),lTarget10);
        }
    else
        {
    ...
    SetLocalInt(OBJECT_SELF,"order_beer", 1);
    DelayCommand(2400.0, SetLocalInt(OBJECT_SELF, "order_beer", 0));
    object oObject = GetFirstObjectInArea(oArea);
        while(GetIsObjectValid(oObject))
            {
             // Destroy any objects tagged "DESTROY"
             if(GetTag(oObject) == "CODI_CUP_6")
             {

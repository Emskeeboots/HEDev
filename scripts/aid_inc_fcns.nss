...
// Return true if sName is either the first, last or full name of the PC. 
// Case insensitive.  This relies on having first and last name set on client enter
int isPCNameMatch(object oPC, string sName) {
	
	string sPC;
	sName = GetStringLowerCase(sName);
	sPC = GetLocalString(oPC, "PC_FIRST_NAME");
	if (GetStringLowerCase(sPC) == sName) return TRUE;
	
	sPC = GetLocalString(oPC, "PC_LAST_NAME");
	if (GetStringLowerCase(sPC) == sName) return TRUE;
...
object GetNearbyPC(string sName, object oPC) {
        
	if (isPCNameMatch(oPC, sName)) {
		//SendMessageToPC(oPC, "Looking for nearby PC '" + sName + "' found self");
		return oPC;
	} 
	int i = 1;
        object oObject   = GetNearestCreature(CREATURE_TYPE_PLAYER_CHAR, PLAYER_CHAR_IS_PC, oPC, i);
        while( oObject!=OBJECT_INVALID && GetDistanceBetween(oPC,oObject)<=MAX_DISTANCE_LOOK ) {
                string sCheck = dlgTokenFirstName(oObject);
		//SendMessageToPC(oPC, "Looking for nearby PC '" + sName + "' found '" + sCheck + "'");
                if (isPCNameMatch(oObject, sName)) {
                        return oObject;
                }  
                oObject   = GetNearestCreature(CREATURE_TYPE_PLAYER_CHAR, PLAYER_CHAR_IS_PC, oPC, ++i);
        }
...
int AIDParseEmote(object oPC, string sSpeech)
{
    ...
    */
    ...
    // Check for sObject being the name of a nearby PC.
    if (GetLocalInt(GetModule(), "AID_EXAMINE_PCS") && !bOneWord) {
	    if (GetStringLeft(sVerb, 4) == "look" || GetStringLeft(sVerb, 7) == "examine") {	    
		    object oOtherPC = GetNearbyPC(sObject, oPC);
		    //SendMessageToPC(oPC, "AID - checking for examining PC '" + sObject + "' verb = '" + sVerb + "'");
		    if (GetIsObjectValid(oOtherPC)) {
			    if (oPC != oOtherPC) {
				    DoExamine(oOtherPC, oPC);
				    return TRUE;   
			    } else {
				    // TESTING
				    DoExamine(oOtherPC, oPC);
    ...
    // flier takes flight. CreatureGetIsFlier handles the switch to a flying appearance/state
    else if(    (   ( (sObject=="flight"||sObject=="wing") && (sVerb=="take" || sVerb=="takes") )
                 || ( bOneWord && (sVerb=="fly" || sVerb=="flies") )
                )
            && CreatureGetIsFlier(oPC,TRUE)
           )
    {
        return TRUE;
    }
    // flier lands
    // this may be one word or part of a more descriptive emote
    else if( //bOneWord &&
            (sVerb=="land" || sVerb=="lands")
            && CreatureGetIsFlying(oPC)
           )
    {
        // Handle Fly/Walk alternate appearances
        int nForm       = GetAppearanceType(oPC);

        int nWalker;
        //int nWalker     = GetLocalInt(oPC, "ALT_WALKER");
        //if(!nWalker)
        //{
            string sWalker  = Get2DAString("appearance_x", "ALT_WALKER", nForm);
                nWalker     = StringToInt(sWalker);
            if(!nWalker)
                nWalker     = -1;
            //SetLocalInt(oPC, "ALT_WALKER", nWalker);
        //}

        // bird lands
        if(nWalker!=-1 && nWalker!=nForm)
        {
            SetCreatureAppearanceType(oPC, nWalker);
            return TRUE;
        }
        else
            return FALSE;
    ...
    else if (IsActionValid(sVerb)&&!bOneWord)
    {
        ...
        if(sObject=="self")
        {
            if (sVerb == "examine" || sVerb == "examines" || sVerb == "look" || sVerb == "looks") {
                ...
                // Send PC deity status message.   Didn't yet have anywhere better to hook this up. 
...
int ActionFork(object oTargetObject, object oPlayer, string sVerb)
{
    ...
    {
        if(GetLocalInt(oTargetObject, "fly") && CreatureGetIsFlier(oPlayer))
            bSuccess = DoMove(oTargetObject, oPlayer, "fly");
        else if(GetLocalInt(oTargetObject, "climb"))
            bSuccess = DoMove(oTargetObject, oPlayer, "climb");
...
string GetNamesOfNearbyAIDObjects(object oPC)
{
    ...
    string sLookDescription= COLOR_MESSAGE + sLookAround1;
      ...
      if(!iCountObjects)
        sLookDescription    =  sLookAroundNothing;

    return sLookDescription;
}

string GetNearbyPCList(object oPC) {
        // feedback string for DoLookAround
        // start description
        string sLookDescription= "";
        ...
        while( oObject!=OBJECT_INVALID && GetDistanceBetween(oPC,oObject)<=MAX_DISTANCE_LOOK ) {
                ...
                if (GetObjectSeen(oObject, oPC)) {
                        // PC name may not be know to looking PC. 
                        string sObjectTag = GetName(oObject); // +  // racial name 

                        if (iCountObjects) {
                                sLookDescription += " and ";
                        }
                        sLookDescription += COLOR_OBJECT;  // different color for PCs?
                        sLookDescription += sObjectTag;
                        sLookDescription += COLOR_MESSAGE;
                        iCountObjects++;
                }
        ...
        if(!iCountObjects) return "";

        sLookDescription = COLOR_MESSAGE+ "You see " + sLookDescription + ".";
...
void DoLookAround(object oPC, int animate=FALSE)
{
    ...
    if (GetLocalInt(GetModule(), "AID_EXAMINE_PCS")) {
            string sPCInfo  =  GetNearbyPCList(oPC);
            if (sPCInfo != "") DelayCommand(0.24, SendMessageToPC(oPC, sPCInfo) );
    }
}
...
string track_get_race(object oCreature, int bFull = FALSE) {
        int nRace = GetRacialType(oCreature);
        string sRace;
        if (bFull) {
                switch (nRace) {
                        case RACIAL_TYPE_ABERRATION: return "An aberration";
                        case RACIAL_TYPE_ALL: return "A creature";
                        ...
                        case RACIAL_TYPE_HUMANOID_GOBLINOID: return "A goblinlike creature";
                        case RACIAL_TYPE_HUMANOID_MONSTROUS: return "A monstrous creature";
                        case RACIAL_TYPE_HUMANOID_ORC: return "An orclike creature";
                        case RACIAL_TYPE_HUMANOID_REPTILIAN: return "A reptilianlike creature";
                        case RACIAL_TYPE_MAGICAL_BEAST: return "A very unusual creature";
                        case RACIAL_TYPE_OUTSIDER: return "An outsider";
                        case RACIAL_TYPE_SHAPECHANGER: return "A possible shapechanger";
                ...
                switch (nRace) {
                        ...
                        case RACIAL_TYPE_DRAGON: return "A large creature";

                        case RACIAL_TYPE_DWARF:
                        case RACIAL_TYPE_HALFLING:
                        case RACIAL_TYPE_GNOME:
                        case RACIAL_TYPE_HUMANOID_GOBLINOID:
                        case RACIAL_TYPE_FEY: return "A small humanoid";

                        case RACIAL_TYPE_ELF:
                        case RACIAL_TYPE_HALFELF:
                        case RACIAL_TYPE_HALFORC:
                        case RACIAL_TYPE_HUMANOID_ORC:
                        case RACIAL_TYPE_HUMAN: return "A humanoid";

                        case RACIAL_TYPE_GIANT: return "A large humanoid";

                        case RACIAL_TYPE_SHAPECHANGER:
                        case RACIAL_TYPE_UNDEAD:
                        case RACIAL_TYPE_HUMANOID_REPTILIAN: return "A strange humanoid";

                        case RACIAL_TYPE_CONSTRUCT: return "An odd thing";
...
// doExaminePC
int doExaminePC(object oTarget, object oPC)
{

        if (!GetObjectSeen(oTarget, oPC)) { 
                SendMessageToPC(oPC, COLOR_MESSAGE + "You can't really see " + GetName(oTarget)+ COLOR_END);
                return 1;
        };

        string sMsg = GetName(oTarget) + " appears to be ";

        string sRace = track_get_race(oTarget, TRUE);
        string sLower = GetStringLeft(sRace, 1);
        string sRest = GetStringRest(sRace, 1);
        sRace = GetStringLowerCase(sLower) + sRest;
	string sGender = "He";
	...
	sMsg += sRace; 
        ...
        //SendMessageToPC(oPC, sMsg + " appears to be " + sRace);
	// of weak. medium, powerful build
	string sBuild = "medium";
	int nStr = GetAbilityScore(oTarget, ABILITY_STRENGTH, FALSE);
	if (nStr >= 17) {
		sBuild = "very powerful";
	} else if (nStr > 14) {
		sBuild = "powerful";
	} else if (nStr < 9) {
		sBuild = "weak";
	}
	sMsg += " of " + sBuild + " build.\n";

	//he/she is unattractive, average looking, attractive, stunning
	string sLooks = "average looking";
	int nCha =  GetAbilityScore(oTarget, ABILITY_CHARISMA, FALSE);
	if (nCha >= 17) {
		sLooks = "stunning";
	} else if (nCha > 14) {
		sLooks = "very attractive";
	} else if (nCha > 11) {
		sLooks = "attractive";
	} else if (nCha < 9)  {
		sLooks = "unattractive";
	}
	sMsg += sGender + " is " + sLooks + " and seems to be in " ;

	// he/she appears to be in excellent/good/poor health
	int nHP = GetCurrentHitPoints(oTarget);
	int nMax = GetMaxHitPoints(oTarget);
	int loss = (nHP * 100)/nMax;
	string sHealth;
	if (loss >= 95) sHealth = "excellent";
	else if (loss > 80) sHealth = "good";
	else if (loss > 30) sHealth = "poor";
	else sHealth = "very poor";
	sMsg += sHealth + " health.";

	SendMessageToPC(oPC, COLOR_MESSAGE + sMsg + COLOR_END);
        return 1;
}
//::///////////////////////////////////////////////
//:: Name: DoExamine
...
int DoExamine(object oTarget, object oPC)
{
    ...
    if (GetIsPC(oTarget) && GetLocalInt(GetModule(), "AID_EXAMINE_PCS")) {
        return doExaminePC(oTarget, oPC);
    }
...
int DoMove(object oTarget, object oPC, string sVerb="climb")
{
    ...
    if(bSuccess || primeMove)
    {
        ...
        if(sVerb=="fly")
        {
            if(CreatureGetIsFlier(oPC, TRUE))
            {
                ActionSpeakString(LIME+GetName(oPC)+" flies through the "+GREEN+GetName(oTarget)+LIME+".");
                location lDest  = GetLocation(oDest);
                object oAreaThis= GetArea(oPC);
                if( GetAreaFromLocation(lDest)==oAreaThis)
                    ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectDisappearAppear(GetLocation(oDest)), oPC, 3.0);
                else
                    AssignCommand(oPC, JumpToLocation(lDest));
            }
            else
            {
                FloatingTextStringOnCreature(LIME+GetName(oPC)+" unsuccessfully attempts to fly.", oPC);
                AssignCommand(oPC, ActionPlayAnimation(ANIMATION_LOOPING_DEAD_FRONT, 1.0, 2.0));
                ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_IMP_DUST_EXPLOSION), oPC);
            }
        }
        else if(sVerb=="climb" || sVerb=="swim" || sVerb=="squeeze" || sVerb=="jump")
        {
    ...
    if ( nCheck )
    {
        ...
        if ( !iPreviousCheck )
        {
            ...
            else
            {
                if(sEvent=="read")
                {
                    int iLang   = GetLanguageID( GetStringLowerCase(GetStringLeft(GetLocalString(oTarget, "readlanguage"),3)) );
                    if(GetSpeaksLanguage(oPC, iLang))
                    {
                        if(iDC>0)
                            iResult = GetIsSkillSuccessful(oPC, iSkill, iDC);
                        else
                            iResult = TRUE;
                        string sFill    = " reading ("+GetLanguageName(iLang)+"). ";
                        sFailResponse   = sFill+sFailResponse;
                        sPassResponse   = sFill+sPassResponse;
                    }
                    else
                    {
                        iResult = FALSE;

                        if( DoSkillCheck(oPC, SKILL_LORE, GetLoreToRecognizeLanguage(iLang)))
                        {
                            string sFill    = " reading ("+GetLanguageName(iLang)+"). ";
                            sFailResponse   = sFill+sFailResponse;
                            sPassResponse   = sFill+sPassResponse;
                        ...
                        {
                            string sFill    = " reading a (language) you don't recognize. ";
                            sFailResponse   = sFill+sFailResponse;
                            sPassResponse   = sFill+sPassResponse;
                        }
                    }

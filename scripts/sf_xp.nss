...
LEVEL_38_MAX_XP = 40,
LEVEL_39_MAX_XP = 40,
LEVEL_40_MAX_XP = 40; // No need for this really
            ...
            if (!GetIsDead(oParty) && fDist >= 0.0 && fDist <= PARTY_DIST)
            {
                ...
                if (oKiller == oParty && nPlayer > 1)
                {
                    ...
                    if (FLOATING_TEXT)
                    {
                        ...
                        FloatingTextStringOnCreature(IntToString(nXPToGive)+" XP", oParty, FALSE);
                    }
                    if (REWARD_GP)
                    {
                        if (FLOATING_TEXT)
                        {
                            FloatingTextStringOnCreature(IntToString(FloatToInt(IntToFloat(nXPToGive) * GP_REWARD_MULTIPLIER))+" GP", oParty, FALSE);
                        }
                        GiveGoldToCreature(oParty, FloatToInt(IntToFloat(nXPToGive) * GP_REWARD_MULTIPLIER));
                    }
                ...
                else
                {
                    ...
                    if (REWARD_GP)
                    {
                        if (FLOATING_TEXT)
                        {
                            DelayCommand(0.1, FloatingTextStringOnCreature(IntToString(FloatToInt(IntToFloat(nXPToGive) * GP_REWARD_MULTIPLIER))+"GP", oParty, FALSE));
                        }
                        GiveGoldToCreature(oParty, FloatToInt(IntToFloat(nXPToGive) * GP_REWARD_MULTIPLIER));
                    }
                    GiveXPToCreature(oParty, nXPToGive);

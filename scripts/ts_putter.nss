...
// For debug enable this (and adjust it if multiplayer...)
void tell( string s ) {
    //SendMessageToPC( GetFirstPC(), "PUTTER: " +  s );
}
...
void tryAnim( string tag, int animIndex, float animDuration, float animSpeed = 1.0 ) {
    if( GetLocalFloat( OBJECT_SELF, "tsAnimDuration" ) == 0.0 ) {
        ...
        string placeName = GetStringLowerCase( GetName( place ) );
...
void pickAnim( object place ) {
    ...
    if( GetObjectType( place ) == OBJECT_TYPE_CREATURE ) {
        switch( Random( 6 ) ) {
            ...
            case 4: setAnim( ANIMATION_LOOPING_CUSTOM17, 8.0 ); break;
            case 5: setAnim( ANIMATION_LOOPING_CUSTOM7, 8.0 ); break;
        }
        return;
    }
    ...
    tryAnim( "chair", ANIMATION_LOOPING_SIT_CHAIR, 500.0 );
    tryAnim( "stool", ANIMATION_LOOPING_SIT_CHAIR, 50.0 );
    tryAnim( "seat", ANIMATION_LOOPING_SIT_CHAIR, 500.0 );
...
//    anim( for people, talk to them.
    ...
    tryAnim( "_ponder", ANIMATION_LOOPING_CUSTOM17, 12.0 );
    ...
    if( GetLocalFloat( OBJECT_SELF, "tsAnimDuration" ) == 0.0 ) {
        ...
        switch( Random( 6 ) ) {
            ...
            case 4: setAnim( ANIMATION_LOOPING_CUSTOM17, 6.0 ); break;
            case 5: setAnim( ANIMATION_LOOPING_CUSTOM7, 6.0 ); break;
        }
    }
}
...
void playAnimation() {
    ...
    if( animIndex == ANIMATION_LOOPING_SIT_CHAIR ) {
        if( !GetIsObjectValid( GetSittingCreature(place) ) ) {
            ...
            ActionDoCommand( DelayCommand( 120.0, resetPlace() ) );
            ActionSit( place );
/*
            switch( d4() ) {
                case 0: case 1: break;

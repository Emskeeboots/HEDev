...
// This is run from a delay as the bonesplc so the parameter is not needed really. 
void destroyBones(object oBonesPlc) {
	object oLootCorpse = GetLocalObject(oBonesPlc, "CORPSE_NODE");

	if(GetIsOpen(oLootCorpse) ||  GetLocalString(oLootCorpse,"IN_USE_BY")!="") {
		// try again 
		DelayCommand(5.0, destroyBones(oBonesPlc));
		return;
	}

	object oBlood = GetLocalObject(oLootCorpse, "PAIRED"); 

	// Need to clean inventory or we get a loot bag- TODO should be based on R7 spawn flag...
	if (GetLocalInt(oLootCorpse, "CorpseDeleteLootOnDecay"))
		NESS_CleanInventory(oLootCorpse);

	//SetPlotFlag(oLootCorpse, FALSE);
	DestroyObject(oLootCorpse);

	//SetObjectIsDestroyable(oBlodd, TRUE, FALSE, FALSE);
	DestroyObject(oBlood);

	//SetObjectIsDestroyable(oBonesPlc, TRUE, FALSE, FALSE);
	DestroyObject(oBonesPlc, 0.2);
}
...
void main()
{
    ...
    //SendMessageToPC(GetFirstPC(), "DEBUG: Corpse_use: corpse_type = " + IntToString(corpse_type)  
    //		    + "  dialog = " + GetLocalString(OBJECT_SELF, "dialog"));
         ...
         )
  {
    ...
    if (GetLocalInt(oHostCorpse, "CorpseDeleteLootOnDecay")) {
	    SetLocalInt(OBJECT_SELF, "CorpseDeleteLootOnDecay", 1);
    }
    // Need to clean the corpse here or we get a loot bag...
    NESS_CleanCorpse(oHostCorpse);
    float fDecay = GetLocalFloat(oHostCorpse, "CorpseDecay");
    ...
    AssignCommand(oBones, DelayCommand(fDecay, destroyBones(oBones)));
  }
  else if (corpse_type & CORPSE_TYPE_PERSISTENT)
  {

...
void main()
{
    ...
    if (!nActive)
    {
        ...
        ActionSpeakString("You charge the crystal");
...
if (GetLocalInt(OBJECT_SELF, "X2_L_PLC_ACTIVATED_STATE")==0)
   {
   ...
   else
   {
    ...
    if (GetTag(GetItemInSlot(INVENTORY_SLOT_RIGHTHAND, oPC)) == "tt_scepter")
        {
        ActionPlayAnimation(ANIMATION_PLACEABLE_DEACTIVATE);
        ActionSpeakString("You discharge the crystal");

void main()
{
    int nActive = GetLocalInt (OBJECT_SELF,"order_stew");
...
if (nActive == 0)
{
    ...
    if (GetGold(oPC) >= 150)
       ...
       CreateObject(OBJECT_TYPE_PLACEABLE,sFoodSpawn2,lTarget3,TRUE);
       CreateObject(OBJECT_TYPE_PLACEABLE,sFoodSpawn2,lTarget4,TRUE);
       ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(iVFX),lTarget3);
       ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(iVFX),lTarget4);
        }
    else
        {
    ...
    SetLocalInt(OBJECT_SELF,"order_stew", 1);
    DelayCommand(2400.0, SetLocalInt(OBJECT_SELF, "order_stew", 0));
    object oObject = GetFirstObjectInArea(oArea);
        while(GetIsObjectValid(oObject))
            {
             // Destroy any objects tagged "DESTROY"
             if(GetTag(oObject) == "QP_DAGGFRD_021")
             {

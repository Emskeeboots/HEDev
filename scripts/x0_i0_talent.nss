...
int TalentSpellAttack(object oIntruder)
{
    ...
    if(GetRacialType(oTarget) == RACIAL_TYPE_UNDEAD)
    {
        if(GetHasSpell(SPELL_UNDEATH_TO_DEATH) && !CompareLastSpellCast(SPELL_UNDEATH_TO_DEATH))
        {
            ...
            ActionCastSpellAtObject(SPELL_UNDEATH_TO_DEATH,oTarget);
            return TRUE;
        }
        ...
        {
            SetLastGenericSpellCast(SPELL_CONTROL_UNDEAD);
            ActionCastSpellAtObject(SPELL_CONTROL_UNDEAD,oTarget);
            return TRUE;
        }
    }
    else if(GetCurrentHitPoints(oTarget) < 101 && GetHasSpell(SPELL_POWER_WORD_KILL) && !CompareLastSpellCast(SPELL_POWER_WORD_KILL))
    {
        SetLastGenericSpellCast(SPELL_POWER_WORD_KILL);
        ActionCastSpellAtObject(SPELL_POWER_WORD_KILL,oTarget);
        return TRUE;
    }
    ...
    */
    ...
    else if(GetCurrentHitPoints(oTarget) < 101 && GetHasSpell(SPELL_POWER_WORD_KILL))
    {
        SetLastGenericSpellCast(SPELL_POWER_WORD_KILL);
        ActionCastSpellAtObject(SPELL_POWER_WORD_KILL,oTarget);
        return TRUE;
...
//       GetRacialType(OBJECT_SELF) != RACIAL_TYPE_CONSTRUCT ||
    ...
    if(nCurrent < nBase ? !(bForce = FALSE) : bForce == TRUE)
    {
        ClearAllActions();
        if(GetRacialType(OBJECT_SELF) != RACIAL_TYPE_UNDEAD)
        {
            talent tUse = GetCreatureTalentBest(TALENT_CATEGORY_BENEFICIAL_HEALING_TOUCH,bForce ? 9 : -1);//in case bForce == TRUE, it will look for lower lvl healing spells
            if(GetIsTalentValid(tUse))
            {
                //MyPrintString("Talent\ Successful Exit");
                ActionUseTalentOnObject(tUse, OBJECT_SELF);//1.70: bkTalentFilter is not appropriate when target is OBJECT_SELF
                return TRUE;
            }
            else
            {
        ...
        {
            if(GetHasSpell(759) && !bForce)//will reserves fullheals for later
            {
            ...
            int nTalent;
            talent tUse = GetCreatureTalentRandom(3,OBJECT_SELF);
            while(GetIsTalentValid(tUse) && nTalent++ < 10)
            {
                if(GetTypeFromTalent(tUse) == TALENT_TYPE_SPELL)
                {
            ...
            tUse = GetCreatureTalentRandom(3,OBJECT_SELF);
            }
            tUse = GetCreatureTalentBest(3,0,OBJECT_SELF);
...
int TalentSneakAttack()
{
     ...
     {//1.71: death attack and BG's sneak also qualify, but only if the creature has class with this feature!
         ...
         if (GetIsObjectValid(oFriend))
         {
             ...
             if(GetIsObjectValid(oTarget) && !GetIsDead(oTarget) && !GetAssociateState(NW_ASC_MODE_DYING, oTarget))
...
int TalentShapechange(object oIntruder = OBJECT_INVALID)
{
    if(GetHasEffect(EFFECT_TYPE_POLYMORPH))
    {
       talent tBreath,tPulse,tUse;
        ...
        if(!GetHasSpellEffect(686) && GetCreatureHasTalent(tUse))
        {
            ActionUseTalentOnObject(tUse,OBJECT_SELF);
            return TRUE;
        }
        ...
        tUse = TalentSpell(SPELL_MESTILS_ACID_BREATH);
        if(d4() == 1 && GetCreatureHasTalent(tUse))
        {
        ...
        {
            tUse = TalentSpell(SPELL_ICE_STORM);
        ...
        if(GetIsTalentValid(tBreath) && GetCreatureHasTalent(tBreath) && GetLocalInt(OBJECT_SELF, "NW_GENERIC_DRAGONS_BREATH") >= 2)
        {
            ...
            ActionUseTalentOnObject(tBreath,oIntruder);
            return TRUE;
        }

...
void main()
{
    ...
    if(MODULE_DEBUG_MODE)
        WriteTimestampedLogEntry("OnChat("+ObjectToString(OBJECT_SELF)+") nMatch("+IntToString(nMatch)+") oShouter("+ObjectToString(oShouter)+")");
    ...
    if (nMatch == -1)
    {
          ...
          )
        {
            if(GetLocalInt(OBJECT_SELF, "IN_CONV"))
            {
                ...
                int nLang       = GetLocalInt(OBJECT_SELF, "CONV_LANGUAGE_SPEAKING");
                if(!nLang)
                    nLang       = GetCurrentLanguageSpoken(OBJECT_SELF);
                if(!nLang)
                    nLang       = LANG_COMMON;
                NPCSpeakForAllToHear(   sBusy,
                                        nLang,
                                        OBJECT_INVALID
                            );

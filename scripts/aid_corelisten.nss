...
void main()
{
    ...
    //DM Handling//
    ///////////////
    else if ( GetStringLeft(sSpeech, 1) == "@" && (GetIsDM(oPC) || MODULE_DEBUG_MODE) )
    {
    ...
    //////////////////
    // !! Debugging commands !! //
    else if (GetStringLeft(sSpeech, 1) == "#" && MODULE_DEBUG_MODE) // ALLOW_PC_DEBUG
    {
        if (sSpeech == "#DebugMode on")
        {
            SetLocalInt(oPC, "DebugMode", 1);
            SendMessageToPC(oPC, (COLOR_MESSAGE + sDebugMode1 + COLOR_END));
        }
        else if (sSpeech == "#DebugMode off")
        {

...
// q_inc_traps -- addition by henesua
void takePitDamage(int nDamage, int bMoved=FALSE);
void takePitDamage(int nDamage, int bMoved=FALSE)
{
    location lLoc   = GetLocation(OBJECT_SELF);
    if(!GetIsObjectValid(GetAreaFromLocation(lLoc)))
    {
        DelayCommand(3.0, takePitDamage(nDamage));
        return;
    }

    PlayVoiceChat(VOICE_CHAT_PAIN3,OBJECT_SELF);
    ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectDamage(nDamage, DAMAGE_TYPE_BLUDGEONING, DAMAGE_POWER_NORMAL), OBJECT_SELF);
...
{
    int nDamage;
    int nDie  = GetLocalInt(oTrapTrigger, "TRAP_DAMAGE_DICE");
    int nDice = GetLocalInt(oTrapTrigger, "TRAP_DAMAGE_DICE_ROLLS");
...
// q_inc_traps -- addition by henesua
int Trap_GetCustomDC(object oTrapTrigger=OBJECT_SELF);
int Trap_GetCustomDC(object oTrapTrigger=OBJECT_SELF)
{
    return  GetLocalInt(oTrapTrigger, "TRAP_DC");
}

void AssociateTrap(string sTag, int nID)
{
    int nN=0;
    object oPlcbl = GetObjectByTag(sTag, nN);
    object oTrap;

    int nTrapObjectType = (OBJECT_TYPE_DOOR|OBJECT_TYPE_PLACEABLE|OBJECT_TYPE_TRIGGER);

    int nT;
    while(GetIsObjectValid(oPlcbl))
    {
        nT = 1;
        oTrap = GetNearestObject(nTrapObjectType, oPlcbl,nT);
        while(GetIsObjectValid(oTrap))
        {
            PrintString("In "+sTag+" Checking for: "+IntToString(nID)+", found: "+GetTag(oTrap));
            if(GetIsTrapped(oTrap)&&!GetLocalInt(oTrap,"TRP_INITIALIZED")&&(GetTrapBaseType(oTrap)==nID))
            {
                break;
            }
        ...
        {
            SetLocalInt(oTrap,"TRP_INITIALIZED", TRUE);
            if(nID==103)
            {
...
void LoadTrapporium()
{
    ...
    AssociateTrap("BHFloorSpikesSm",112);
    AssociateTrap("BHFloorSpikesLg",113);
    AssociateTrap("BHWallSpikes",114);
    ...
    AssociateTrap("Metalgroundspikes1",122);
    AssociateTrap("Metalgroundspikes2",123);
}
...
void Drown(object oPC, object oWaterArea)
{
    if(     GetLocalInt(OBJECT_SELF,"TRP_TRIGGERED")==1
        &&  (GetIsInSubArea(oPC, oWaterArea)||!GetIsObjectValid(oWaterArea))
      )
    {
        int nD = GetLocalInt(oPC,"DROWNING");
        if(nD==12) //Beginning of the 13th round
        {

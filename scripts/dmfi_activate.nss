...
void main()
{
    ...
    if (GetStringLeft(sItemTag,8) == "hlslang_")
    {
        ...
        SetLocalInt(oUser, LANGUAGE_SPOKEN_ID, StringToInt(ssLanguage));
        SetLocalString(oUser, LANGUAGE_SPOKEN_NAME, GetName(oItem));
    ...
    if (GetStringLeft(sItemTag, 5) == "dmfi_")
    {
        ...
        if (sItemTag == "dmfi_exploder")
        {
            ...
            if(!GetIsObjectValid(GetItemPossessedBy(oOther, "spc_manual"))) CreateItemOnObject("spc_manual", oOther);

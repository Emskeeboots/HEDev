      ...
      )
    {
        ...
        while(oBuddy!=OBJECT_INVALID)
        {
            ...
            if(     GetObjectSeen(oBuddy)
                &&  !GetIsEnemy(oBuddy)
                &&( (GetLocalInt(OBJECT_SELF,sBuddyID)+(nDelay*GetLocalInt(GetModule(), "IGMINUTES_PER_RLMINUTE"))) < nNow )
              )
            {
                if(bPC)
                {
                    ...
                    ActionDoCommand(NPCSpeakForAllToHear(sSay, nLang, OBJECT_SELF));
                    ActionWait(7.0);
                    bSuccess=TRUE;
                    break;
                }
...
void main()
{
    if(MODULE_DEBUG_MODE)
        SendMessageToPC(GetFirstPC(),"spawn_sc_patrol");
    ...
    if(MODULE_DEBUG_MODE)
        SendMessageToPC(GetFirstPC(),"PATROL  Script("+IntToString(nPatrolScript)+") Stop("+ObjectToString(oStop)+")  Next("+IntToString(nStopNumber)+")");
...
// -------------------------------------------
    ...
    else if(nPatrolScript == 11)
    {
        ...
        if(MODULE_DEBUG_MODE)
            SendMessageToPC(GetFirstPC(),"FARM("+ObjectToString(oField)+")  Name("+GetName(oField)+")");

...
void main()
{
    ...
    if (GetAILevel() == AI_LEVEL_VERY_LOW)
    {
        ...
        if(GetLocalInt(GetArea(OBJECT_SELF), "AREA_PC_COUNT")<1)
            return;
    }
    ...
    else if(GetLastPerceptionSeen())
    {
        ...
        else if(bSpecial)
        {
            ...
            else if( DoSkillCheck(oPercep, SKILL_ANIMAL_EMPATHY, GetLocalInt(OBJECT_SELF, "ANIMAL_EMPATHY_DC")) )
            {
    ...
    */
	...
	dblvlstr(DEBUGLEVEL_AI, msg+" "+sSense+" react("+IntToString(nReactionType)+")");
	//SendMessageToPC(GetFirstPC(),msg+" "+sSense+" react("+IntToString(nReactionType)+")");
    }

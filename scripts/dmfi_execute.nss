...
//:: 2012.06.01 Magus - adjustments to dicebag to work with Arnheim skills
//:: 2014.05.30 Henesua - adjustments to DoXPFunction so that it uses Vives' XP system
...
void RollDemBones(object oUser, int iBroadcast, int iMod = 0, string sAbility = "", int iNum = 1, int iSide = 20)
{
    ...
    if (GetLocalInt(oUser, "dmfi_dice_no_animate")!=1)
    {
        ...
        case 72: AssignCommand(oUser, PlayAnimation(ANIMATION_LOOPING_TALK_NORMAL, 1.0, 5.0f)); break;
        case 73: AssignCommand(oUser, PlayAnimation(ANIMATION_LOOPING_TALK_LAUGHING, 1.0, 5.0f)); break;
        case 93:
        case 77: AssignCommand(oUser, PlayAnimation(ANIMATION_LOOPING_GET_LOW, 1.0, 5.0f)); break;
        case 75:
        case 79:
        case 99:
        case 81:
        case 87: AssignCommand(oUser, PlayAnimation(ANIMATION_LOOPING_GET_MID, 1.0, 5.0f)); break;
        case 82: SetActionMode(oUser, ACTION_MODE_STEALTH, TRUE);
                 AssignCommand(oUser, PlayAnimation(ANIMATION_LOOPING_CUSTOM10, 1.0, 5.0f));
        break;
        case 98:
        case 86: AssignCommand(oUser, PlayAnimation(ANIMATION_FIREFORGET_DODGE_SIDE, 1.0)); break;
        case 83: AssignCommand(oUser, ActionPlayAnimation(ANIMATION_LOOPING_TALK_FORCEFUL, 1.0, 5.0)); break;
        ...
        case 88:
                if(FindSubString(GetTag(GetItemInSlot(INVENTORY_SLOT_LEFTHAND, oUser)),"instrument")!=-1)
                {
                    SetLocalObject(oUser, "DMFI_USE_INSTRUMENT", GetItemInSlot(INVENTORY_SLOT_LEFTHAND, oUser));
                    ExecuteScript("do_instrument", oUser);
                }
                else
                    AssignCommand(oUser, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_DUR_BARD_SONG), oUser, 6.0f));
        break;

        case 91: AssignCommand(oUser, PlayAnimation(ANIMATION_FIREFORGET_STEAL, 1.0)); break;
        case 92:
        case 96: AssignCommand(oUser, PlayAnimation(ANIMATION_LOOPING_LOOK_FAR, 1.0, 5.0f)); break;
...
void DoDiceBagFunction(int iDice, object oUser, int iDMOverride = 0)
{
    ...
    case 93: iNum = 1; iSide = 20; sAbility="Set Trap Check, "; iMod = GetSkillRank(SKILL_SET_TRAP, oUser); break;
...
//special case for filling of water and sewer regions
    ...
    TLResetAreaGroundTiles(oArea, iXAxis, iYAxis);
    TLChangeAreaGroundTiles(oArea, nEffect , iXAxis, iYAxis, ZFinalAxis);

    SetLocalInt(oArea, "TILEMAGIC_EFFECT_SET", nEffect);
}
...
void DoNewDMThingy(int iChoice, object oUser)
{
    ...
    case 79: TLResetAreaGroundTiles(GetArea(oUser), iXAxis, iYAxis);
             DeleteLocalInt(GetArea(oUser), "TILEMAGIC_EFFECT_SET");
            break;
    ...
    case 89: TLResetAreaGroundTiles(GetArea(oUser), iXAxis, iYAxis);
             DeleteLocalInt(GetArea(oUser), "TILEMAGIC_EFFECT_SET");
            break;
...
void DoXPFunction(int iXP, object oUser)
{
    ...
    if (iParty==1)
    {
        ...
        while (GetIsObjectValid(oPartyMember))
        {
            ...
            FloatingTextStringOnCreature(sFloating, oPartyMember, FALSE);
            //FloatingTextStringOnCreature(sFloating + ": " + IntToString(iReward), oPartyMember, FALSE);
            XPRewardByType("",oPartyMember,iReward,nXPType);
    ...
    else  //single player code
    {
        ...
        // reward?
        if(iReward>-1)
            XPRewardByType("",oTarget,iReward,nXPType);
        // penalty.....
        else
            XPStoreByType(oTarget, iReward, XP_TYPE_ADJUSTMENT);
...
void CreateEffects(int iEffect, location lEffect, object oUser)
{
    ...
    case 110: ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_DUR_FLIES), oTarget, fDuration); break;
...
void DoEmoteFunction(int iEmote, object oUser)
{
    ...
    switch (iEmote)
    {
    case 1: // point forward
        AssignCommand(oTarget, PlayAnimation(ANIMATION_LOOPING_CUSTOM16, 1.0, 2.0));
    break;
    case 2: // beckon to follow
        AssignCommand(oTarget, PlayAnimation(ANIMATION_LOOPING_CUSTOM9, 2.0, 2.4));
    break;
    case 3: // thoughtful pose
        AssignCommand(oTarget, PlayAnimation(ANIMATION_LOOPING_CUSTOM17, 1.0, 120.0));
    break;
    case 4: // crouch low
        AssignCommand(oTarget, PlayAnimation(ANIMATION_LOOPING_CUSTOM10, 1.0, 10000.0));
    break;
    case 5: // lies down
        ...
        AssignCommand(oTarget, PlayAnimation( ANIMATION_LOOPING_CUSTOM18, 1.0, 10000.0));
    break;
    case 6: // cowers
        AssignCommand(oTarget, PlayAnimation(ANIMATION_LOOPING_CUSTOM5, 1.0, 120.0));
    break;
    case 7: // arms crossed
        AssignCommand(oTarget, PlayAnimation(ANIMATION_LOOPING_CUSTOM7, 1.0, 120.0));
    break;

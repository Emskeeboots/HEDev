...
const string Q_ACP_KNOWN    = "KNOWN";

const string Q_ACP_ARCANE_STRING  = "arcane";
const string Q_ACP_ASSASSIN_STRING= "monkey grip";
...
const string Q_ACP_HUNG_STRING    = "bear claw";
const string Q_ACP_MUAY_STRING    = "tiger fang";
const string Q_ACP_SHAO_STRING    = "dragon palm";
const string Q_ACP_SHOTO_STRING   = "sun fist";
const string Q_ACP_WARRIOR_STRING = "warrior";
const string Q_ACP_NORMAL_STRING  = "normal";
const string Q_ACP_LARGE_STRING   = "large";
...
void Q_ACPSetStyle(object oPC, int nPheno, string sStyle = "", int bOverride = FALSE);
...
void Q_ACPSetStyle(object oPC, int nPheno, string sStyle = "", int bOverride = FALSE)
{
    ...
    {
        SendMessageToPC(oPC,RED+"You may not change fighting styles while swimming.");
        return;
    }
    ...
    // only styles that the player knows can be used
    if( sStyle!=""
        && sStyle!=GetSkinString(oPC,"PHENOTYPE_NATURAL_NAME")
        && !GetSkinInt(oPC,Q_ACP_KNOWN+sStyle)
      )
    {
        if(MODULE_DEBUG_MODE)
            SendMessageToPC(oPC, RED+"Unknown Style: " + sStyle);
    ...
    if (GetModuleSwitchValue(MODULE_SWITCH_ENABLE_REQUIRE_DEMONBLADE_MULTIWEAPON) && !bOverride)
    {
    ...
    if (GetModuleSwitchValue(MODULE_SWITCH_ENABLE_UNARMED_STYLES_LIMITATION) && !bOverride)
    {
...
int Q_ACPGetPhenoFromString(string sText)
{
    ...
    if( sText==GetStringLowerCase(Q_ACP_ASSASSIN_STRING) )
    {
    ...
    if (sText == GetStringLowerCase(Q_ACP_LARGE_STRING))
    {
...
void Q_ACPCheckChat(object oPC, string sText)
{
    ...
    if (GetStringLeft(sText, GetStringLength(Q_ACP_KEYWORD)) == GetStringLowerCase(Q_ACP_KEYWORD))
    {
        ...
        string sStyle = GetStringRight(sText, GetStringLength(sText) - GetStringLength(Q_ACP_KEYWORD) -1);
        int nPheno = Q_ACPGetPhenoFromString(sStyle);
        if (nPheno != -1)
        {
            Q_ACPSetStyle(oPC, nPheno, sStyle, MODULE_DEVELOPMENT_MODE);
            SetPCChatMessage();
        }
    }

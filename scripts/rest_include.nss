...
/*
    ...
    Example Rules included in this config file are as follows:
        ...
        : comfortable rest (regain ALL hitpoints and spells) has the following requirements:
            - campfire: required in cold conditions (eg. outdoors at night)
...
// Magus' Camp System - campfires and campsites (also include Magus' Food System)
...
const int REST_SPELL_LIMIT_INT      = 1;
const int REST_SPELL_LIMIT_WIS      = 1;
const int REST_SPELL_LIMIT_CHA      = 1;
...
// 1+ = hours required between rests OR  0 = no required time between rests
const int HOURS_BETWEEN_RESTS   = 2;
...
// configure your tests for resting here. returns a bit flag [FILE: rest_config]
...
// Configure the amount of damage to apply after a rest. [FILE: rest_config]
...
// Returns TRUE if enough time has passed since last rest [FILE: rest_config]
int GetIsTimeToRest(object oPC=OBJECT_SELF);
// Returns TRUE if oPC is cold (needs a fire to rest) [FILE: rest_config]
int GetIsCold(object oPC=OBJECT_SELF);
// Returns TRUE if oPC needs a bed to rest [FILE: rest_config]
int GetNeedsBedToRestComfortably(object oPC=OBJECT_SELF);

// ........ You may have already defined similar functions ........

// Returns TRUE if oPC has a bed to rest on [FILE: rest_config]
...
int ResterRequirements(int nTest)
{
    ...
    // require that the character's breathing is unimpacted
    else if(nTest==5 && GetLocalString(OBJECT_SELF,"UNDERWATER_ID")!="")
    {
        // is their breathing impacted?
        if( GetLocalInt(OBJECT_SELF,"UNDERWATER_EFFECTS")&IMPACT_BREATHING )
        {
            nResult |= REST_POSTPONE; // current rest action will be cancelled
            SetLocalInt(OBJECT_SELF, CAUSE_OF_REST_FAILURE, nTest);// setup explanation for why the PC can not rest
        }
...
string ResterFailureText(int nTestFailed)
{
    ...
    int time = (GetLocalInt(OBJECT_SELF, NEXT_REST_TIME)-GetTimeCumulative())/GetLocalInt(GetModule(), "IGMINUTES_PER_RLMINUTE");
    if(time<=0)
        time = 2;
    switch(nTestFailed)
    {
        ...
        case 1:
            return (    RED+"You may only rest once every "+IntToString(HOURS_BETWEEN_RESTS)+" game hours. "
...
void ResterFinishesRest()
{
    ...
    if(MODULE_DEBUG_MODE)
        SendMessageToPC(OBJECT_SELF, "REST: ResterFinishesRest()");
    ...
    if(!GetRestUnrestricted())
        SetLocalInt(OBJECT_SELF, NEXT_REST_TIME, GetTimeCumulative()+(HOURS_BETWEEN_RESTS*60) );

    // VFX like horns, necklaces, quivers, glowing eyes etc....
    DeleteLocalInt(oPC, "vfx_do_op");
    ExecuteScript("_vfx_do_op", oPC);

    // THE MAGUS' INNOCUOUS FAMILIARS
    SetLocalInt(oPC, "pets_tmp_op", 1);
    ExecuteScript("pets_do_op", oPC);

    // update PC persistence
    Data_SavePC(OBJECT_SELF, TRUE);
}
...
// (eg. you may want certain items to "activate" their special powers)
    ...
    int nType = GetBaseItemType(oConsume);
    // Eat Food
    if(GetIsFood(oConsume))
    {
        CreatureEatsFood(oConsume);
        bDestroy = FALSE;
    }
    return bDestroy;
}
...
int GetIsTimeToRest(object oPC=OBJECT_SELF)
{
    ...
    // Adding a check here to make sure the value is sane.
    int nNext = GetLocalInt(oPC, NEXT_REST_TIME); 
    int nCur = GetTimeCumulative();
    int nMaxNext = nCur +(HOURS_BETWEEN_RESTS*60);
    if (nNext > nMaxNext) {
        WriteTimestampedLogEntry("Rest - fixing next rest " + IntToString(nNext) 
                + " time now = " + IntToString(nCur) + " for " + GetName(oPC));
        nNext = nMaxNext;
        SetLocalInt(oPC, NEXT_REST_TIME, nNext);
    }
...
int GetIsCold(object oPC=OBJECT_SELF)
{
    ...
    int wet = (     GetLocalString(oPC,"UNDERWATER_ID")!=""
                ||  GetLocalString(oPC,"SWAMP_ID")!=""
                ||  GetWeather(oArea)==WEATHER_RAIN
              );
    ...
    if(     GetLocalInt(oArea, AREA_COLD) // area flagged as cold?
        ...
        || (wet && !CreatureGetIsAquatic(oPC)) // non-aquatic who is wet
      )
    {
        nExposed    = TRUE;
...
int GetRestUnrestricted(object oRester=OBJECT_SELF)
{
    ...
    if(      GetLocalInt(oRester, REST_UNRESTRICTED) )
        ...
        return FALSE;
//    else if( GetLocalInt(GetModule(), REST_RESTRICTED) )
//        return FALSE;
...
void RestLimitHP()
{
    ...
    if (nDamage > 0) {
	    SetLocalInt(OBJECT_SELF, "nwnx_tmp_op", 4);
	    SetLocalInt(OBJECT_SELF, "nwnx_tmp_val", GetCurrentHitPoints(OBJECT_SELF) - nDamage);
	    ExecuteScript("nwnx_do_op", OBJECT_SELF);
    }
}
...
void ResterAddConsumable(object oConsumable)
{
    int nCount  = GetLocalInt(OBJECT_SELF, "CONSUMABLE_COUNT")+1;
    SetLocalInt(OBJECT_SELF, "CONSUMABLE_COUNT", nCount);
    SetLocalObject(OBJECT_SELF, "CONSUMABLE_"+IntToString(nCount), oConsumable);
}

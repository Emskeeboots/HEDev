void main()
{
    int nActive = GetLocalInt (OBJECT_SELF,"order_cheese");
...
if (nActive == 0)
{
    ...
    if (GetGold(oPC) >= 80)
        ...
        CreateObject(OBJECT_TYPE_PLACEABLE,sFoodSpawn4,lTarget7,TRUE);
        CreateObject(OBJECT_TYPE_PLACEABLE,sFoodSpawn3,lTarget8,TRUE);
        ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(iVFX),lTarget7);
        ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(iVFX),lTarget8);
        }
    else
        {
    ...
    SetLocalInt(OBJECT_SELF,"order_cheese", 1);
    DelayCommand(2400.0, SetLocalInt(OBJECT_SELF, "order_cheese", 0));
    object oObject = GetFirstObjectInArea(oArea);
        while(GetIsObjectValid(oObject))
            {
             // Destroy any objects tagged "DESTROY"
             if(GetTag(oObject) == "tt_cheese")
              {
                 DelayCommand(2400.0, DestroyObject(oObject));
              }
             else if(GetTag(oObject) == "CODI_LONGBREAD")
              {
                 DelayCommand(2400.0, DestroyObject(oObject));
              }
